...
struct WeatherRepository {
    ...
    static let celciusUnits = "metric"
    
    static func getWeather (geolocation: Geolocation, onClosure: @escaping (_ weather: Dictionary <String, Any>?, _ error: Error?) -> Void) {
        Alamofire.request("http://api.openweathermap.org/data/2.5/weather?lat=\(geolocation.latitude)&lon=\(geolocation.longitude)&appid=\(appId)&units=\(celciusUnits)").responseJSON(completionHandler: { (response) in
